<?php
    ...
    function check_token($received_token, $received_data) {
        $env = parse_ini_file('./../.env');
        $token_generic = $env['SECRET_KEY'].$_SERVER['SERVER_NAME'];
        $token = hash('sha256', $token_generic.$received_data);
    
        if ($received_token != $token){
            return false;
        }
    
        list($tokenDate, $userData) = explode("_", $received_data);
    ...
    // Envio de correo
    function send_password_mail($mail, $user, $id) {
        $tokenhandler = new token_handler();
        $token = $tokenhandler -> create_token(array('userId' => $id), 'resetPassword', $id, '10 minutes', false);
        $verify_url = 'https://Joyeria.local.com/resetpassword/' . $token;
    
        $verify_mail = new mail_handler();
        $verify_mail -> to = $mail;
        $verify_mail -> subject = "Resetear contraseña";
    ...
    if(isset($_POST) && isset($_POST['type']) && $_POST['type'] == 'requestResetMail') {
        // Obtener datos
        ...
        if (empty($result[0])) {
            $response = array(
                'status' => 'denied',
                'message' => 'No existe ninguna cuenta registrada con el correo electrónico introducido.'
            );
            die(json_encode($response));
        }

        // Enviar correo electrónico
        if (empty(send_password_mail($mail, $result[0]['nombre_usuario'], $result[0]['id']))) {
            $response = array(
                'status' => 'error',
    ...
    // Cambiar contraseña
    } else if (isset($_POST) && isset($_POST['type']) && $_POST['type'] == 'changePassword') {
        // Obtener datos
        ...
        // Verificar token
        $tokendata = $tokenhandler -> verify_token($token, 'resetPassword');
        if (empty($tokendata)) {
            $response = array(
                'status' => 'denied',
        ...
        $conn = new db_handler();
        $conn -> execute("UPDATE registros_user SET contraseña = ? WHERE id = ?", 'ss', [$hashed_pass, $id]);

        // Anular el resto de tokens
        $tokenhandler -> cancel_user_tokens($id, 'resetPassword');
